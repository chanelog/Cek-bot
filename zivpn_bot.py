...
def is_screenshot_used(img_hash: str) -> bool:
    """Cek apakah screenshot sudah pernah dipakai"""
    if not img_hash:
        return False
    db = Path(SS_HASH_DB)
    if not db.exists():
        return False
    return img_hash in db.read_text().splitlines()

def mark_screenshot_used(img_hash: str, user_id: int):
    """Simpan hash screenshot ke database"""
    if not img_hash:
        return
    db = Path(SS_HASH_DB)
    db.parent.mkdir(parents=True, exist_ok=True)
    with open(db, "a") as f:
        f.write(f"{img_hash}|{user_id}|{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
async def handle_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Cegah kirim screenshot berulang saat menunggu input user/pass
    ...
    if is_screenshot_used(img_hash):
        admin_tg = CFG.get("ADMIN_TG", "@admin")
        await update.message.reply_text(
            f"🚫 <b>Screenshot Sudah Pernah Digunakan!</b>\n\n"
            f"Screenshot ini sudah pernah dipakai untuk membuat akun sebelumnya.\n"
            f"Kirim screenshot pembayaran yang baru & berbeda.\n\n"
            f"Jika ada masalah hubungi admin: {admin_tg}",
            parse_mode="HTML"
        )
    ...
    if ok is True:
        # Tandai screenshot sudah dipakai
        mark_screenshot_used(img_hash, user.id)
        try: os.remove(img_path)
        except: pass

        # Simpan state — tunggu user input username
        ctx.user_data["waiting_username"] = True
        ctx.user_data["ss_verified"]      = True

        await update.message.reply_text(
            f"✅ <b>Pembayaran Terverifikasi!</b>\n\n"
            f"Sekarang tentukan akun UDP kamu:\n\n"
            f"Ketik <b>username</b> yang kamu inginkan:\n"
            f"<i>(Huruf kecil, angka, minimal 4 karakter)</i>",
            parse_mode="HTML"
        )

    elif ok is None:
        # OCR tidak tersedia — forward ke admin verifikasi manual
        # Simpan hash dulu (belum ditandai used, tunggu admin konfirm)
        ctx.user_data["pending_hash"]      = img_hash
        ...
        await update.message.reply_text(
            "⏳ Screenshot diterima.\n"
            "Admin akan memverifikasi dalam beberapa menit. Harap tunggu 🙏"
        )
            ...
            try:
                await ctx.bot.send_photo(
                    ...
                    caption = (
                        ...
                        f"✅ /konfirm_{user.id} — Approve (bot minta user/pass ke pembeli)\n"
                        f"❌ /tolak_{user.id}   — Tolak"
                    ),
                    parse_mode="HTML"
                )
            except: pass
        ...
        await update.message.reply_text(
            ...
            f"• Tujuan transfer ke nomor yang benar\n\n"
...
async def cmd_konfirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Tandai screenshot sudah dipakai (jika ada hash)
    pending_hash = pdata.get("pending_hash", "")
    if pending_hash:
        mark_screenshot_used(pending_hash, target_uid)

    ctx.bot_data.pop(f"pending_{target_uid}", None)

    # Kirim notif ke pembeli — minta input username & password
    try:
        # Simpan state di bot_data agar handle_text bisa akses
        ctx.bot_data[f"konfirm_{target_uid}"] = {
            "paket_id": paket_id,
            "ss_verified": True,
        }
        await ctx.bot.send_message(
        ...
        await update.message.reply_text(
            f"✅ Pembayaran dikonfirmasi. Bot sudah minta user/pass ke pembeli.",
            parse_mode="HTML"
        )
    except Exception as e:
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_action == "add_free" and is_admin(user.id):
        await handle_admin_text(update, ctx)
        return

    # ── Admin: hapus akun ────────────────────────────────────
    if admin_action == "del_akun" and is_admin(user.id):
        await handle_admin_del(update, ctx)
        return
    ...
    if ctx.user_data.get("waiting_username") and ctx.user_data.get("ss_verified"):
        ...
        if len(username) < 4:
            await update.message.reply_text(
                "❌ Username minimal <b>4 karakter</b>. Coba lagi:",
                parse_mode="HTML"
            )
            return
        if not re.match(r"^[a-z0-9_]+$", username):
            await update.message.reply_text(
                "❌ Username hanya boleh huruf kecil, angka, dan underscore. Coba lagi:",
                parse_mode="HTML"
            )
            return
        if user_exists(username):
            await update.message.reply_text(
                f"❌ Username <code>{username}</code> sudah dipakai. Pilih username lain:",
                parse_mode="HTML"
            )
            return
        ...
        await update.message.reply_text(
            ...
            f"Sekarang ketik <b>password</b> yang kamu inginkan:\n"
            f"<i>(Minimal 6 karakter, bebas kombinasi huruf & angka)</i>",
            parse_mode="HTML"
        )
        return
    ...
    if ctx.user_data.get("waiting_password") and ctx.user_data.get("ss_verified"):
        ...
        if len(password) < 6:
            await update.message.reply_text(
                "❌ Password minimal <b>6 karakter</b>. Coba lagi:",
                parse_mode="HTML"
            )
            return
        ...
        for admin_id in CFG.get("ADMIN_IDS", []):
            try:
                await ctx.bot.send_message(
                    ...
                    f"📅 Expired  : {akun['exp']}\n"
                    f"✅ Dibuat sesuai request pembeli",
                    parse_mode="HTML"
                )
            except: pass

        log.info(f"Akun dibuat: {username} untuk TG user {user.id}")
        return
    ...
    if action == "cek_akun":
        ...
        if not info:
            await update.message.reply_text(
                f"❌ Akun <code>{text}</code> tidak ditemukan.",
                parse_mode="HTML"
            )
        else:
            await update.message.reply_text(
