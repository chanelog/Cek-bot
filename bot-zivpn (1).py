#!/usr/bin/env python3
# ================================================================
#   OGH-ZIV STORE BOT v2.0
#   Terhubung ke ogh-ziv.sh (chanelog/Socks)
#   - Pembayaran DANA wajib → konfirmasi admin → akun dibuat
#   - Admin bisa buat akun gratis (tanpa bayar)
#   - Tampil IP server setiap buat akun
#   - Panel akun aktif otomatis tampil setelah create
#   - Harga: 7hr=3rb | 15hr=6rb | 30hr=10rb
# ================================================================
...
# ── Path — sesuai ogh-ziv.sh ────────────────────────────────────
DIR       = "/etc/zivpn"
UDB       = f"{DIR}/users.db"        # user|pass|exp|kuota|info
PAKETDB   = f"{DIR}/paket.db"
ORDERDB   = f"{DIR}/orders.db"
BOTCONF   = f"{DIR}/bot.conf"        # BOT_TOKEN, CHAT_ID
DANACONF  = f"{DIR}/dana.conf"       # DANA_NO, DANA_NAME
ADMINCONF = f"{DIR}/admin.conf"      # ADMIN_USERNAME
DOMAINF   = f"{DIR}/domain.conf"
MAXLOGDB  = f"{DIR}/maxlogin.db"
CFGJSON   = f"{DIR}/config.json"     # zivpn config (auth.config = passwords)
LOGFILE   = f"{DIR}/bot.log"
IPCACHE   = f"{DIR}/server.ip"
...
DANA        = {"no": _dc.get("DANA_NO", "-"), "name": _dc.get("DANA_NAME", "-")}

if not BOT_TOKEN:
    raise SystemExit("❌ BOT_TOKEN kosong! Jalankan installer dulu.")
...
def ks(n):
    return "Unlimited" if int(n) == 0 else f"{n} GB"

def gen_oid():
    return "ORD" + str(int(time.time()))[-5:] + "".join(random.choices(string.digits, k=4))
...
def get_server_ip():
    # cache 1 jam
    try:
        if os.path.exists(IPCACHE) and time.time() - os.path.getmtime(IPCACHE) < 3600:
            ip = open(IPCACHE).read().strip()
    ...
    try:
        open(IPCACHE, "w").write(ip)
    except: pass
    return ip

def get_host():
    try: return open(DOMAINF).read().strip()
    except: return get_server_ip()

def get_port():
    try:
        cfg = json.load(open(CFGJSON))
        v = cfg.get("listen", cfg.get("port", ":7300"))
        return str(v).lstrip(":")
    except: return "7300"
...
def paket_load():
    ...
    try:
        for line in open(PAKETDB):
            p = line.strip().split("|")
            if len(p) >= 5:
                hasil.append({"nama":p[0],"hari":int(p[1]),"harga":int(p[2]),"kuota":int(p[3]),"maxlogin":int(p[4])})
    except: pass
    return hasil or PAKET_DEFAULT
...
def order_save(o):
    with open(ORDERDB, "a") as f:
        f.write(f"{o['id']}|{o['uid']}|{o['uname']}|{o['paket']}|"
...
def order_load():
    ...
    try:
        for line in open(ORDERDB):
            p = line.strip().split("|")
            if len(p) >= 12:
                hasil.append({"id":p[0],"uid":int(p[1]),"uname":p[2],"paket":p[3],
                    "hari":int(p[4]),"harga":int(p[5]),"kuota":int(p[6]),"maxlogin":int(p[7]),
                    "user":p[8],"pass":p[9],"status":p[10],"tgl":p[11]})
    except: pass
    return hasil
...
# ================================================================
#  AKUN VPN — sinkron ogh-ziv.sh / zahidbd2/udp-zivpn
# ================================================================
...
def vpn_reload():
    ...
    Format ogh-ziv / zahidbd2: config.json["auth"]["config"] = ["pass1","pass2",...]
    """
    try:
        ...
        if "auth" in cfg and isinstance(cfg["auth"], dict):
            ...
            cfg["auth"] = {"type": "password", "config": pws}

        json.dump(cfg, open(CFGJSON, "w"), indent=2)
        subprocess.run(["systemctl", "restart", "zivpn"], capture_output=True)
        log.info(f"VPN reloaded — {len(pws)} akun aktif")
    except Exception as e:
        log.error(f"vpn_reload error: {e}")
...
def panel_text():
    ...
    if not aktif:
        lines.append("_Belum ada akun aktif._")
    else:
        for i, p in enumerate(aktif[:30], 1):
            user = p[0]; exp = p[2] if len(p) > 2 else "-"
            try:
                sisa = (datetime.strptime(exp, "%Y-%m-%d") - datetime.now()).days
                sisa_txt = f"{sisa}hr"
            except: sisa_txt = "-"
            lines.append(f"`{i:>2}.` `{user:<18}` exp:{exp} ({sisa_txt})")
        if len(aktif) > 30:
            lines.append(f"_...+{len(aktif)-30} lainnya_")
    return "\n".join(lines)

def kirim_panel_admin(label=""):
    msg = (f"🔄 *{label}*\n\n" if label else "") + panel_text()
    to_admin(msg)

# ================================================================
#  TAMPILAN USER
# ================================================================
EMO = {"pending_payment":"⏳ Menunggu Bayar","waiting_confirm":"🔍 Verifikasi Admin",
       "done":"🎉 Aktif","rejected":"❌ Ditolak"}
...
def show_cek_order(uid):
    ...
    if not milik:
        ...
        send(uid, "📋 *CEK ORDER*\n\nKamu belum punya order.", reply_markup=kb)
        return
    text = "📋 *RIWAYAT ORDER KAMU*\n\n"
    for o in milik[-5:]:
        st = EMO.get(o["status"], o["status"])
        text += (
            f"🆔 `{o['id']}`\n"
...
def show_help(uid):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
        types.InlineKeyboardButton("🛒 Beli Sekarang", callback_data="m_beli"),
        types.InlineKeyboardButton(f"📞 Admin @{ADMIN_UNAME}", url=f"https://t.me/{ADMIN_UNAME}"),
        types.InlineKeyboardButton("« Kembali", callback_data="m_back"),
    )
    send(uid,
        "❓ *CARA BELI VPN*\n\n"
        ...
        "4️⃣ Kirim *📸 screenshot bukti transfer* ke bot ini\n"
        ...
        f"📞 Keluhan / konfirmasi → @{ADMIN_UNAME}",
        reply_markup=kb
    )

def show_admin_contact(uid):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(f"💬 Chat @{ADMIN_UNAME}", url=f"https://t.me/{ADMIN_UNAME}"))
    kb.add(types.InlineKeyboardButton("« Kembali", callback_data="m_back"))
    send(uid,
        f"📞 *HUBUNGI ADMIN*\n\n"
        f"👤 Admin : @{ADMIN_UNAME}\n"
...
def kirim_akun_user(o, exp):
    ip   = get_server_ip()
    host = get_host()
    port = get_port()
    kb   = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(f"📞 Admin @{ADMIN_UNAME}", url=f"https://t.me/{ADMIN_UNAME}"))
    send(o["uid"],
        ...
        f"📞 Ada masalah? → @{ADMIN_UNAME}",
        reply_markup=kb
    )

def kirim_akun_admin(o, exp):
    """Detail akun yang dikirim ke admin setelah create (untuk semua metode)"""
    ip   = get_server_ip()
    host = get_host()
    port = get_port()
    send(ADMIN_ID,
...
def on_photo(msg):
    ...
    if not o:
        send(uid,
            ...
            "Ketik /start untuk mulai order baru."
        )
        return
    order_update_status(o["id"], "waiting_confirm")
    ...
    send(uid,
        ...
        f"🆔 Order : `{o['id']}`\n"
        f"📦 Paket : {o['paket']}\n\n"
        f"⏱ Admin sedang memverifikasi *(< 5 menit)*\n"
...
def on_message(msg):
    ...
    # ── Admin commands ───────────────────────────────────────────
    elif is_admin(uid) and cmd == "/confirm" and args:
        cmd_confirm(uid, args[0])
    elif is_admin(uid) and cmd == "/reject" and args:
        cmd_reject(uid, args[0])
    elif is_admin(uid) and cmd == "/listorder":
        cmd_listorder(uid)
    elif is_admin(uid) and cmd == "/listakun":
        cmd_listakun(uid)
    elif is_admin(uid) and cmd == "/hapusakun" and args:
        u = args[0]
        akun_hapus(u)
        send(uid, f"✅ Akun `{u}` dihapus & VPN direload.")
        kirim_panel_admin(f"Akun `{u}` dihapus")
    elif is_admin(uid) and cmd == "/listpaket":
        cmd_listpaket(uid)
    elif is_admin(uid) and cmd == "/newpaket":
        STATE[uid] = {"step": "np_nama"}
        send(uid, "📦 Nama paket baru:\nContoh: `UDP 30 Hari`")
    elif is_admin(uid) and cmd == "/delpaket" and args:
        try: cmd_delpaket(uid, int(args[0]) - 1)
        except: send(uid, "❌ Format: `/delpaket 1`")
        ...
        show_paket_free(uid)
    elif is_admin(uid) and cmd == "/panel":
        send(uid, panel_text())
...
def on_callback(call):
    ...
    if   data == "m_beli":  show_paket(uid)
    elif data == "m_order": show_cek_order(uid)
    elif data == "m_help":  show_help(uid)
    elif data == "m_admin": show_admin_contact(uid)
    elif data == "m_back":  show_start(uid, call.from_user.first_name)

    elif data.startswith("beli_"):
        idx = int(data[5:])
        daftar = paket_load()
        if not (0 <= idx < len(daftar)): return
        p = daftar[idx]
        ...
        if not o or o["status"] != "pending_payment":
            send(uid, "❓ Order tidak ditemukan / sudah diproses."); return
        order_update_status(oid, "waiting_confirm")
        notif_ke_admin(o)
        send(uid,
            f"✅ *Admin sudah diberitahu!*\n\n"
            f"🆔 Order : `{oid}`\n"
            f"⏱ Estimasi konfirmasi *< 5 menit*\n\n"
            f"Lebih dari 5 menit? Hubungi @{ADMIN_UNAME}"
        )
        ...
        if not (0 <= idx < len(daftar)): return
        p = daftar[idx]
        STATE[uid] = {"step":"adm_free_user","kuota":p["kuota"],"maxlogin":p["maxlogin"],"hari":p["hari"],"paket":p["nama"]}
        send(uid, f"📦 Paket: *{p['nama']}* ({p['hari']} hari)\n\n👤 Masukkan *username*:")

# ================================================================
#  ADMIN FUNCTIONS
# ================================================================
def show_paket_free(uid):
    daftar = paket_load()
    if not daftar:
        STATE[uid] = {"step":"adm_free_user","kuota":0,"maxlogin":2,"hari":0,"paket":"Custom"}
        send(uid, "👤 Masukkan *username* akun gratis:")
        return
    kb   = types.InlineKeyboardMarkup(row_width=1)
    text = "📦 *Pilih paket untuk akun GRATIS:*\n\n"
    for i, p in enumerate(daftar):
        text += f"• {p['nama']} ({p['hari']} hari) — Rp {rp(p['harga'])}\n"
        kb.add(types.InlineKeyboardButton(f"📦 {p['nama']} ({p['hari']} hari)", callback_data=f"adm_free_{i}"))
    send(uid, text, reply_markup=kb)
...
def cmd_confirm(uid, oid):
    ...
    if o["status"] == "done":
        send(uid, "⚠️ Order sudah selesai."); return
    exp = akun_create(o)
    order_update_status(oid, "done")
    kirim_akun_user(o, exp)
    kirim_akun_admin(o, exp)
    log.info(f"confirm oid={oid} user={o['user']} exp={exp}")
    # Panel akun aktif
    kirim_panel_admin(f"Akun baru aktif: {o['user']} (exp:{exp})")
...
def cmd_listakun(uid):
    try: lines = [l.strip() for l in open(UDB) if l.strip()]
    except: lines = []
    if not lines: send(uid, "👤 Belum ada akun."); return
    today   = datetime.now().strftime("%Y-%m-%d")
    aktif   = sum(1 for l in lines if len(l.split("|")) >= 3 and l.split("|")[2] >= today)
    expired = len(lines) - aktif
    ip      = get_server_ip()
    text = (f"👤 *LIST AKUN VPN*\n"
            f"🖥️ IP: `{ip}` | Port: `{get_port()}`\n"
            f"Total: {len(lines)} | ✅ Aktif: {aktif} | ❌ Exp: {expired}\n\n")
    for l in lines[:30]:
        p = l.split("|")
        if len(p) < 3: continue
        st = "✅" if p[2] >= today else "❌"
        text += f"{st} `{p[0]}` exp:{p[2]}\n"
    if len(lines) > 30: text += f"_...+{len(lines)-30} lainnya_"
    send(uid, text)

def cmd_listpaket(uid):
    daftar = paket_load()
    if not daftar: send(uid, "📦 Belum ada paket."); return
    text = "📦 *LIST PAKET*\n\n"
    for i, p in enumerate(daftar):
        text += (f"*{i+1}. {p['nama']}*\n"
                 f"   ⏰ {p['hari']} hari | 💰 Rp {rp(p['harga'])} | "
...
def cmd_delpaket(uid, idx):
    daftar = paket_load()
    if not (0 <= idx < len(daftar)): send(uid, "❌ Nomor paket tidak valid."); return
    nama = daftar[idx]["nama"]; daftar.pop(idx); paket_save(daftar)
...
def handle_state(msg, text):
    ...
    if step == "input_username":
        ...
        if akun_exists(text):
            send(uid, "❌ Username *sudah dipakai*, coba nama lain."); return
        state["vuser"] = text; state["step"] = "input_password"; STATE[uid] = state
        send(uid, f"✅ Username: `{text}`\n\n🔑 Masukkan *password* (min 6 karakter):")

    elif step == "input_password":
        if len(text) < 6:
            send(uid, "❌ Password minimal *6 karakter*!"); return
        state["vpass"] = text
        STATE.pop(uid, None)
        daftar = paket_load()
        p = daftar[state["pidx"]]
        ...
        o = {"id":oid,"uid":uid,"uname":uname,"paket":p["nama"],"hari":p["hari"],
             "harga":p["harga"],"kuota":p["kuota"],"maxlogin":p["maxlogin"],
             "user":state["vuser"],"pass":state["vpass"],"status":"pending_payment",
             "tgl":datetime.now().strftime("%Y-%m-%d %H:%M")}
        order_save(o)
        kb = types.InlineKeyboardMarkup(row_width=1)
        kb.add(types.InlineKeyboardButton("📸 Sudah Transfer, Beritahu Admin", callback_data=f"bukti_{oid}"))
        kb.add(types.InlineKeyboardButton(f"📞 Admin @{ADMIN_UNAME}", url=f"https://t.me/{ADMIN_UNAME}"))
        send(uid,
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"📸 Kirim *screenshot bukti transfer* ke chat ini\n"
            f"atau tekan tombol di bawah\n\n"
            f"⚠️ *Akun TIDAK akan dibuat sebelum pembayaran dikonfirmasi!*\n"
            f"⏰ Order batal otomatis dalam *30 menit*",
            reply_markup=kb
        )
        to_admin(
            f"📝 *ORDER BARU — BELUM BAYAR*\n\n"
            f"🆔 Order   : `{oid}`\n"
            f"👤 Pembeli : @{uname} (`{uid}`)\n"
    ...
    # ── Admin buat akun gratis ────────────────────────────────────
        ...
        if not re.match(r"^[a-zA-Z0-9_]{3,20}$", text):
            send(uid, "❌ Username tidak valid!"); return
        if akun_exists(text):
            send(uid, "❌ Username sudah ada, coba yang lain."); return
        state["vuser"] = text; state["step"] = "adm_free_pass"; STATE[uid] = state
        send(uid, f"✅ Username: `{text}`\n\n🔑 Masukkan *password*:")

    elif step == "adm_free_pass":
        if len(text) < 4:
            send(uid, "❌ Password minimal 4 karakter!"); return
        state["vpass"] = text
        if state.get("hari", 0) > 0:
            STATE.pop(uid, None)
            _exec_buat_akun_free(uid, state)
        else:
            state["step"] = "adm_free_hari"; STATE[uid] = state
            send(uid, f"✅ Pass: `{text}`\n\n⏰ Masa aktif *(hari)*:")

    elif step == "adm_free_hari":
        if not text.isdigit() or int(text) < 1:
            send(uid, "❌ Masukkan angka hari!"); return
        state["hari"] = int(text); STATE.pop(uid, None)
        _exec_buat_akun_free(uid, state)

    # ── Set DANA ──────────────────────────────────────────────────
    elif step == "set_dana_no":
        state["new_no"] = text; state["step"] = "set_dana_name"; STATE[uid] = state
        send(uid, f"✅ No DANA: `{text}`\n\n👤 Masukkan *nama pemilik* (A/N):")

    elif step == "set_dana_name":
        DANA["no"] = state["new_no"]; DANA["name"] = text
        dana_save(); STATE.pop(uid, None)
        send(uid, f"✅ *DANA diperbarui!*\n\n💳 No  : `{DANA['no']}`\n👤 A/N : *{DANA['name']}*")

    # ── Tambah paket ──────────────────────────────────────────────
    elif step == "np_nama":
        state["np_nama"] = text; state["step"] = "np_hari"; STATE[uid] = state
        send(uid, f"📦 *{text}*\n\n⏰ Masa aktif *(hari)*:")
    elif step == "np_hari":
        if not text.isdigit() or int(text) < 1: send(uid, "❌ Masukkan angka!"); return
        state["np_hari"] = int(text); state["step"] = "np_harga"; STATE[uid] = state
        send(uid, f"⏰ {text} hari\n\n💰 Harga *(Rupiah)*:")
    elif step == "np_harga":
        if not text.isdigit(): send(uid, "❌ Masukkan angka!"); return
        state["np_harga"] = int(text); state["step"] = "np_kuota"; STATE[uid] = state
        send(uid, f"💰 Rp {rp(text)}\n\n📦 Kuota GB *(0=unlimited)*:")
    elif step == "np_kuota":
        if not text.isdigit(): send(uid, "❌ Masukkan angka!"); return
        state["np_kuota"] = int(text); state["step"] = "np_maxlogin"; STATE[uid] = state
        send(uid, f"📦 {ks(int(text))}\n\n🔒 Max login device:")
    elif step == "np_maxlogin":
        if not text.isdigit() or int(text) < 1: send(uid, "❌ Masukkan angka!"); return
        daftar = paket_load()
        daftar.append({"nama":state["np_nama"],"hari":state["np_hari"],"harga":state["np_harga"],
                        "kuota":state["np_kuota"],"maxlogin":int(text)})
        paket_save(daftar); STATE.pop(uid, None)
        send(uid,
            f"✅ *Paket ditambahkan!*\n\n"
...
def _exec_buat_akun_free(uid, state):
    """Eksekusi buat akun gratis oleh admin — tampilkan IP + panel aktif"""
    o = {
        "id": gen_oid(), "uid": uid, "uname": "admin",
        "paket": state.get("paket", f"FREE {state['hari']} Hari"),
        "hari": state["hari"], "harga": 0,
        "kuota": state.get("kuota", 0), "maxlogin": state.get("maxlogin", 2),
        "user": state["vuser"], "pass": state["vpass"],
        "status": "done", "tgl": datetime.now().strftime("%Y-%m-%d %H:%M"),
    }
    order_save(o)
    exp  = akun_create(o)
    ip   = get_server_ip()
    ...
    # Tampilkan panel akun aktif
    kirim_panel_admin(f"Akun gratis dibuat: {o['user']} (exp:{exp})")
...
if __name__ == "__main__":
    ...
    paket_init()
    ip = get_server_ip()
    log.info(f"OGH-ZIV Bot v2.0 | admin_id={ADMIN_ID} | @{ADMIN_UNAME} | dana={DANA['no']} | ip={ip}")
    print(f"✅ OGH-ZIV Bot v2.0 berjalan")
    print(f"   Admin  : @{ADMIN_UNAME} ({ADMIN_ID})")
    print(f"   Server : {ip}:{get_port()}")
    print(f"   DANA   : {DANA['no']} ({DANA['name']})")
